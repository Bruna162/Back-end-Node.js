...
router.get("/ListaAnime/:id_anime", async (req, res) => {
  const { id_anime } = req.params;

  try {
    const conexao = await mysql.bancoDados();
    const [resultado] = await conexao.execute(
      "SELECT id_anime, titulo, sinopse, estudio, imagem_url, episodios_totais AS episodios, genero FROM animes WHERE id_anime = ?",
      [id_anime]
    );
    await conexao.end();
    ...
    res.status(500).send("Erro ao buscar anime: " + erro.message);
  }
});
